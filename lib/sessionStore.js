...
class SessionStore {
  ...
  async saveSessionToMongo() {
    try {
      ...
      // Add folder with filter to exclude cache files
      // The filter is a RegExp that matches files to INCLUDE
      // We use a negative lookahead to exclude common cache directories
      const filter = /^(?!.*(Cache|Code Cache|GPUCache|Service Worker|ShaderCache|Crashpad|Default\\Cache|Default\\Code Cache)).*$/;
      
      zip.addLocalFolder(SESSION_DIR, undefined, filter);
