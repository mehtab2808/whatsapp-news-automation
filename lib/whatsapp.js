    ...
    const attemptConnection = () => {
      ...
      client.on('ready', async () => {
        try {
          ...
          console.log(`📤 Sending message to group: "${group.name}" (ID: ${group.id._serialized})...`);
          const msg = await client.sendMessage(group.id._serialized, message);
          console.log('✅ Message sent command issued.');
          
          // Wait for message acknowledgment (optional but good for debugging)
          console.log('⏳ Waiting for message acknowledgment...');
          try {
            console.log(`🆔 Message ID: ${msg.id.id}`);
            console.log(`📝 Message Status: ${msg.ack}`);
            console.log(`📄 Message Info:`, JSON.stringify(msg, null, 2));
          } catch (e) {
            console.log('⚠️ Could not get message details:', e.message);
          }

          console.log('✅ Message sent to WhatsApp group successfully.');
          
          // Wait a moment before destroying to ensure message is sent
          // Increased from 3000 to 10000 to ensure delivery
          console.log('⏳ Waiting 15 seconds to ensure delivery...');
          await new Promise(resolve => setTimeout(resolve, 15000));
